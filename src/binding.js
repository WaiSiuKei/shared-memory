  ...
  return (
    function (Module) {
      ...
      function UTF8ArrayToString(heapOrArray, idx, maxBytesToRead) {
        ...
        if (endPtr - idx > 16 && heapOrArray.buffer && UTF8Decoder) {return UTF8Decoder.decode(heapOrArray.subarray(idx, endPtr));} else {
          var str = '';
          while (idx < endPtr) {
      ...
      if (Module['locateFile']) {
        wasmBinaryFile = 'binding.wasm';
        if (!isDataURI(wasmBinaryFile)) {wasmBinaryFile = locateFile(wasmBinaryFile);}
      } else {wasmBinaryFile = new URL('binding.wasm', import.meta.url).toString();}
